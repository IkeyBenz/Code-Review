...
module.exports = {
  ...
  UpdateRequest: async (req, res) => {
    try {
      ...
      const isAsker = String(request.answerer._id) === String(req.user._id);
      const isAnswerer = String(request.asker._id) === String(req.user._id);
      if (isAnswerer || isAsker) {
        ...
        // If the update contains an edited request from the asker, update
        // the date_request_updated property
