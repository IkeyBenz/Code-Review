    ...
    if (user) {
      user.comparePassword(req.body.password, (error, matched) => {
        ...
        if (matched) {
          ...
          if (req.is('application/json')) {
            return res.json({ success: `You already had an account on code-review. You're now signed in as ${req.body.email}` });
          }
          return res.redirect(`/dashboard?success=You already had an account on code-review. You're now signed in as ${req.body.email}`);
        } if (req.is('application/json')) {
          return res.json({ error: 'An account with that email address already exists.' });
        }
        return res.redirect('/signup?error=An account with that email address already exists.');
      });
    } else {
